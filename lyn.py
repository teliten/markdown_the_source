...
file = open('/root/atham/THE_MISSING_LINKS')
dict = file.readlines()
dict2 = {}
for line in dict:
 three = line.split(',')
 try:
  if three[1] not in dict2:
    dict2.setdefault(three[1],three[2])
 except:
    print three
...
def bysolve2(link):
  ...
  try:
   if link in dict2 and link != dict2[link]:
    return bysolve(dict2[link])
  except:
    return link

  if 'https://www.google.com/url?q=' in link:
    return bysolve(url.unquote(link[29:]))
  if '/x/c?' in link:
    #print "X/C?"
    url = 'http://gmass.co'+link[link.index('/x/c'):]
    link = url.replace('%3D','=')
