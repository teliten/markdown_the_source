...
def bysolve2(link):
  ...
  if 'click.php' in bso.geturl():
    ...
    return bysolve(bso.geturl())
  return bso.geturl()
myd = ['/lamc.la','/www.lamc.la','/m.lamc.la','/reallyhim.com','/en.reallyhim.com','/fromthemachine.org','/suez.fromtheachine.org','/lamc.la','/unduecoercion.org','/unduecoercion.com','/www.unduecoercion.org','/www.unduecoercion.com']

maskmyd = ['/www.maraegi.tk','/www.lamc.la','/www.fromthemachine.org','/en.reallyhim.com','/img.izing.ml','/cinoize.github.io']
...
def bymask(link):
   ...
   for d in myd:
    if d in link and 'niallbunting' not in link:
     newlink = "http://www.niallbunting.com/url-multi-redirect/?a="
     for y in maskmyd:
       newlink+=link.replace(d,y)+','
     link=newlink[:-1]
